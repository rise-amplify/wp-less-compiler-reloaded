...
* Escaped all HTML outputs in admin UI using `esc_attr()` to mitigate XSS risk
* Sanitized all user input using `sanitize_text_field()` before processing
* Updated `SettingsHtml.php`, `Processor.php`, and `SettingsStyle.php` for compliance with WordPress security best practices
* Prepared plugin for potential adoption or safe forking due to lack of recent maintenance

== Upgrade Notice ==
= 1.3.1 =
This version includes essential security fixes to prevent XSS vulnerabilities. It is strongly recommended for all users currently using version 1.3.0 or earlier.
